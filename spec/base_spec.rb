RSpec.describe ActiveForm::Base do
  describe ".new" do
  ...
  describe ".from_params" do
    ...
    it "populates attributes from a params hash" do
      form = UserForm.from_params(params[:user].permit!)
    ...
    it "populates nested object attributes" do
      form = UserForm.from_params(params[:user])
    ...
    it "populates array attributes of objects" do
      ...
      expect(form.contacts.size).to eq(3)
    ...
    it "populates nested indexed arrays of attributes" do
      ...
      expect(form.contacts.size).to eq(2)
      expect(form.contacts[0].name).to eq("Amber")
      expect(form.contacts[0].number).to eq("123")
    ...
    it "populates a derived form" do
      ...
      form = ChildForm.from_params(params[:user])
    ...
    it "populates uploaded file attributes" do
      form = FileUploadForm.from_params(params[:user])
  ...
  describe ".from_model" do
    ...
    it "populates attributes from a model with a has_many" do
      ...
      expect(form.contacts.size).to eq(1)
  ...
  describe ".model_name" do
    ...
    it "uses the class name minus the `Form` suffix as the params key" do
      ...
      form = OrderForm.from_params(order_params["order"])
  ...
  describe "#attributes" do
    it "returns a hash of attributes" do
      ...
      expect(form.attributes).to eq("number" => "12345", "id" => 1, "_destroy" => nil)
    end
  end
